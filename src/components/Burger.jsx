...
export const Burger = ({ isActive, setIsModal, isDark, handleSetDark }) => {
    console.log(isActive);
    return (
        <div
            className={!isActive ? "fixed w-[100%] h-[100hv] right-0 top-0 z-10 translate-x-[300%] transition-all" :
            ...
            <div className="w-[100%] h-[100vh] bg-white relative overflow-y-auto" onClick={(event) => event.stopPropagation()}>
                <img src={cross} alt="cross" className='absolute right-3 top-3 cursor-pointer' onClick={() => setIsModal(false)} />
                <div className="px-[32px] pb-[38px] flex flex-col justify-between h-[100%]">
                    <ul className='inline-flex flex-col pt-[70px] gap-9 uppercase font-syncopate font-bold text-[20px] leading-[21px]'>
                        <li>
                            <a
                                href="#"
                                className="md:text-[32px] md:leading-[33px]"
                                onClick={() => setIsModal(false)}
                            >
                                About Us</a>
                        </li>
